...
public:
	...
	static int constexpr DEFAULT_PIRATES_SPAWN_INTERVAL = 30000;  ///< 30000 milliseconds == 30 seconds. This is pirates spawn intreval of time
	...
	int createBattlePirateShip();

	int buyConvoyShip(ShipTypes typeOfConvoyShip);
	Mission& sellConvoyShip(int id);

	Mission& buyWeaponInPlace (int shipID, WeaponTypes type, WeaponPlaces place);

	Mission& sellWeaponInPlace (int shipID, WeaponTypes type, WeaponPlaces place);

	Mission& destroyShipByID(int shipID, Teams team);
	Mission& setWeightToShip(int shipID, int weight);
